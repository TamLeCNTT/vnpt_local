...
const ImPortMayNo = (props) => {
  ...
  const handleExport = async () => {
    ...
    worksheet.mergeCells(`A${centerRow.number}:P${centerRow.number}`);
    ...
    dataRow.forEach((row) => {
      if (row.getCell(2).value === "Tổng cộng") {
        ...
        worksheet.mergeCells(`A${noteRowIndex}:P${noteRowIndex + 1}`);
    ...
    // **Gộp ô**
    const mergeColumns = ["H", "I", "K", "L"]; // Các cột cần gộp
